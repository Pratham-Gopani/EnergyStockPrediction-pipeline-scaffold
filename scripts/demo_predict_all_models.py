"""Demo/smoke test for the uploaded models ONLY -- no news scraping, no
network calls, no yfinance. Builds a synthetic multi-day daily history for one
company (enough rows for the Keras LSTM's 15-day sequence and the 7-day rolling
...
This is the fastest way to check "do all the uploaded models load and produce a
number" without waiting on Google News / Yahoo Finance / HuggingFace downloads.
It does NOT validate that models flagged Unverified in the output table are
numerically correct -- see prediction/feature_engineering.py's module docstring
for why that can't be confirmed from the uploaded files alone.
"""
...
def main() -> None:
    ...
    unverified_names = sorted({p.model_name for p in predictions if p.unverified})
    if unverified_names:
        print(
            f"\nNOTE: {', '.join(unverified_names)} are UNVERIFIED -- see "
