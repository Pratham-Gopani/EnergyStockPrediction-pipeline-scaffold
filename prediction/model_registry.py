...
-- a scikit-learn joblib artifact (stock_news_rf_model.pkl, a MultiOutputRegressor
predicting [Open, Close] in one call, paired with its own input StandardScaler)
and Keras 3 `.keras` LSTM models -- and runs each one's prediction through a
common interface. This is what lets the orchestration layer get, sanitize, and
store a prediction from EVERY uploaded model per company/day, not just one.

All backend-specific imports (joblib, torch, tensorflow) are lazy, inside the
loader functions, so importing this module doesn't require any of them to be
installed.
"""
...
from prediction.feature_engineering import FEATURE_SETS, feature_sequence, latest_feature_row, stock_news_feature_row
...
class ModelSpec:
    def __init__(self, raw: dict):
        ...
        self.feature_builder = raw.get("feature_builder", "engineered")  # "engineered" | "stock_news_row"
        ...
        self.input_scaler_file = raw.get("input_scaler_file")
        self.output_index = raw.get("output_index")  # set for a multi-output model (e.g. [Open, Close] in one call)
...
def _load_scaler(path):
    """Loads a scaler artifact -- used both for a Keras model's target (output)
    scaler and for stock_news_rf_model's input scaler; same joblib.load either way.
    """
    import joblib  # lazy: heavy dependency
...
class ModelRunner:
    ...
    def _get_target_scaler(self, spec: ModelSpec):
        ...
        return self._registry.get_or_create(f"scaler:{spec.target_scaler_file}", lambda: _load_scaler(path))

    def _get_input_scaler(self, spec: ModelSpec):
        if not spec.input_scaler_file:
            return None
        path = self._model_path(spec.input_scaler_file)
        return self._registry.get_or_create(f"scaler:{spec.input_scaler_file}", lambda: _load_scaler(path))
    ...
    def predict_one(
        ...
        try:
            ...
            if spec.backend in ("sklearn", "xgboost"):
                if spec.feature_builder == "stock_news_row":
                    today_row = daily_history.iloc[-1].to_dict() if not daily_history.empty else {}
                    row = stock_news_feature_row(today_row)
                else:
                    row = latest_feature_row(daily_history, ticker, ticker_codes)
                vector = np.array([[row[col] for col in columns]], dtype=float)

                input_scaler = self._get_input_scaler(spec)
                if input_scaler is not None:
                    vector = input_scaler.transform(vector)

                raw_prediction = np.asarray(model.predict(vector), dtype=float)
                if raw_prediction.ndim == 1:
                    raw_prediction = raw_prediction.reshape(1, -1)

                # Inverse-transform BEFORE indexing by output_index -- a
                # multi-output target scaler is fit jointly across all output
                # columns, so it needs the full (1, n_outputs) vector, not one
                # already-selected scalar.
                target_scaler = self._get_target_scaler(spec)
                if target_scaler is not None:
                    raw_prediction = target_scaler.inverse_transform(raw_prediction)

                if spec.output_index is not None:
                    value = float(raw_prediction[0][spec.output_index])
                else:
                    value = float(raw_prediction.reshape(-1)[0])
