...
def run_zero_shot(text: str, candidate_labels: list[str], multi_label: bool = False) -> dict:
    """Locked call into the shared zero-shot pipeline. HuggingFace's fast (Rust)
    tokenizers aren't safe to call from multiple threads at once -- scheduler.tasks
    processes companies concurrently, so without this lock two overlapping calls
    into this shared pipeline raise RuntimeError: Already borrowed. Exposed so
    nlp.headline_strength's optional zero-shot blend reuses the same lock instead
    of calling the pipeline directly.
    """
    classifier = get_zero_shot_pipeline()
    with ModelRegistry().lock_for(_ZERO_SHOT_REGISTRY_KEY):
        return classifier(text, candidate_labels=candidate_labels, multi_label=multi_label)
...
def _classify_zero_shot(text: str) -> dict[str, float]:
    ...
    result = run_zero_shot(text, hypothesis_texts, multi_label=False)
...
def _classify_fine_tuned(text: str) -> dict[str, float]:
    ...
    with ModelRegistry().lock_for(_FINE_TUNED_REGISTRY_KEY):
